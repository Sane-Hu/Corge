...
@pytest.fixture
def coding_agent(provider, tool_runtime, approval_gateway, context_service, knowledge_graph):
    return CodingAgent(
        provider=provider,
...
def test_dispatch_write_uses_content(coding_agent, provider, tool_runtime, tmp_path):
    ...
    # Mock tool runtime
    tool_runtime.write.return_value = ToolResult(action=ToolAction.WRITE, output="ok", success=True)
    ...
    tool_runtime.write.assert_called_once_with(Path(str(tmp_path / "test.txt")), "hello world")
...
def test_dispatch_edit_extracts_old_new(coding_agent, provider, tool_runtime, tmp_path):
    ...
    tool_runtime.edit.return_value = ToolResult(action=ToolAction.EDIT, output="ok", success=True)
    ...
    tool_runtime.edit.assert_called_once_with(Path(str(tmp_path / "test.txt")), "old line", "new line")
...
def test_execute_step_max_actions_exceeded(coding_agent, provider, tool_runtime, tmp_path):
    resp1 = MagicMock()
    resp1.content = "```json\n" + json.dumps({
        "done": False,
        "actions": [{
...
def test_read_deduplication_skips_second_read(coding_agent, provider, tool_runtime, tmp_path):
    resp1 = MagicMock()
    resp1.content = "```json\n" + json.dumps({
        "done": False,
        "actions": [{
    ...
    provider.chat.side_effect = [resp1, resp2]
    tool_runtime.read.return_value = ToolResult(action=ToolAction.READ, output="ok", success=True)
...
def test_evaluate_completion_returns_true_on_satisfied_criteria(coding_agent, provider):
    ...
    bundle = ContextBundle(specification=spec, plan=Plan(()), repository_context=MagicMock(), engineering_profile=MagicMock())
