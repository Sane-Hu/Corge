...
class SpecificationAgent:
    ...
    def analyze_specification_gaps(
        ...
        instruction = (
            ...
            "1. You MUST return an empty array `[]` if the specification is a simple, self-contained task (e.g., printing a string, formatting data, creating a basic script). Do NOT invent gaps just to be thorough.\n"
            "2. The user input originates from a UI workspace called the 'canvas'. This is just the input screen, "
            "and has NO relation to files named 'canvas_draft.py' or similar in the repository unless explicitly requested by the user. "
            "Do NOT assume any file named 'canvas_draft.py' is the target of the specification.\n"
            "3. Ignore minor implementation details or minor ambiguities (e.g., whether to create a new file or modify an existing one, "
            "file naming choices, punctuation inconsistencies, or standard library usage). Do NOT generate semantic gaps for such non-blocking choices; "
            "assume reasonable defaults can be resolved during planning/coding.\n"
            "Return ONLY a JSON array of objects with a 'topic' key, or `[]` if there are no critical blocking gaps.\n\n"
            f"<draft_text>\n{canvas_text}\n</draft_text>"
        )
        ctx_bundle = self._context_service.load_context(
            RepositoryContext(root=Path("."))
