...
class PlanningAgent:
    ...
    def generate_technical_plan(self, specification: Specification) -> TechnicalPlan:
        prompt = (
            ...
            "Focus STRICTLY on system architecture, database schema changes, and API contracts. "
        ...
        return TechnicalPlan(content=response.content, specification_ref=specification.title)
    ...
    def generate_procedural_steps(
        ...
        if not steps:
            steps.append(ProceduralStep(identifier="step-1", description="Execute technical plan"))
